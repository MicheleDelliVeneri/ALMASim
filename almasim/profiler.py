...
def prepare_2d_arrays(Npix):
    ...
    return beam, totsampling, dirtymap, noisemap, robustsamp, Gsampling, Grobustsamp, GrobustNoise

def prepare_baselines(Nant, nH, Hcov, ):
        """
...
def set_baselines(Nbas, antnum, B, u, v, antPos, trlat, trdec, H, wavelength):
    ...
    for currBas in bas2change:
        ...
        # component).
        u[currBas, :] = -(
            B[currBas, 0] * H[0] + B[currBas, 1] * H[1]
        )
...
def _grid_uv(Nbas, totsampling, Gsampling, noisemap, u, v, Nphf, Gains, Noise, 
            robust, nH, imsize):
    """
    ...
    for nb in bas2change:
        ...
        # (half-plane).
        goodpix = np.where(
            np.logical_and(np.abs(pixU) < Nphf, np.abs(pixV) < Nphf)
        )[0]
    ...
    robfac = (
        (5.0 * 10.0 ** (-robust)) ** 2.0
        * (2.0 * Nbas * nH)
        / np.sum(totsampling**2.0)
    )
    return pixpos, totsampling, Gsampling, noisemap, UVpixsize, baseline_phases, bas2change, robfac

def set_beam(robfac, totsampling, robustsamp, Gsampling, GrobustNoise, Grobustsamp, noisemap, beam, Nphf):
...
def _prepare_model(Npix, img, Nphf, Np4, zooming):
    modelim = [
        np.zeros((Npix, Npix), dtype=np.float32) for i in [0, 1]
    ]
...
@delayed
def image_channel(img, wavelength, Npix, Nant, Hcov, nH, noise, antPos, robfac,
                  trlat, trdec, Diameters, imsize, Xmax, lfac, distmat, Nphf, Np4, zooming):
    beam, totsampling, dirtymap, noisemap, robustsamp, Gsampling, Grobustsamp, GrobustNoise = prepare_2d_arrays(Npix)
    Nbas, B, basnum, basidx, antnum, Gains, Noise, H, u, v, ravelDims = prepare_baselines(Nant, nH, Hcov)
    Noise = set_noise(noise, Noise)
    B, u, v = set_baselines(Nbas, antnum, B, u, v, antPos, trlat, trdec, H, wavelength)
    pixpos, totsampling, Gsampling, noisemap, UVpixsize, baseline_phases, bas2change, robfac = _grid_uv(Nbas, totsampling, Gsampling, noisemap, u, v, Nphf, Gains, Noise, robust, nH, imsize)
    beam, beamScale, robustsamp, GrobustNoise, Grobustsamp = set_beam(robfac, totsampling, robustsamp, Gsampling, GrobustNoise, Grobustsamp, noisemap, beam, Nphf)
    ...
    modelfft, modelim = set_primary_beam(header, distmat, wavelength, Diameters, modelim, modelimTrue)
    dirtymap, modelvis, dirtyvis = observe(dirtymap, GrobustNoise, modelfft, Grobustsamp, beamScale)
    del beam, totsampling, noisemap, robustsamp, Gsampling, Grobustsamp, GrobustNoise
    del Nbas, B, basnum, basidx, antnum, Gains, Noise, H, u, v, ravelDims
    del pixpos, baseline_phases, bas2change
    return modelim[0], dirtymap, modelvis, dirtyvis
...
class Interferometer(QObject):
    ...
    def run_interferometric_sim(self):
        ...
        scattered_channels = [client.scatter(self.skymodel[i]) for i in range(self.skymodel.shape[0])]
        modelcube = []
        modelvis = []
        dirtycube = []
        dirtyvis = []
        ...
        output_array1, output_array2, output_array3, output_array4 = compute(
        delayed_model, delayed_modelvis, delayed_dirty, delayed_dirtyvis
        )
        print(output_array1.shape, output_array2.shape, output_array3.shape, output_array4.shape)

        
if __name__ == '__main__':
    skymodel = np.ones((956, 256, 256))
    ra = 120
    dec = 30
    central_freq = 142
    band_range = 1
