...
class TestAlmaUtils(unittest.TestCase):
    ...
    def test_query_alma_failure(self):
        ...
        with patch("app.alma_utils.TAPService") as mock_tap_service:
            ...
            self.assertIn(
                "Query failed", str(context.exception.last_attempt.exception())
            )
    ...
    def test_scientific_categories_filter(self):
        ...
        science_filters = {"scientific_categories": ["Category1", "Category2"]}
    ...
    def test_spatial_resolution_filter(self):
        ...
        science_filters = {"spatial_resolution": (0.5, 1.5)}
        query = generate_query_for_science(science_filters)
    ...
    def test_velocity_resolution_filter(self):
        ...
        science_filters = {"velocity_resolution": (0.1, 1.0)}
        query = generate_query_for_science(science_filters)
    ...
    def test_query_by_targets_service_error(self):
        ...
        targets = [("NGC253", "uid://A001/X122/X1")]
        with patch(
            "app.alma_utils.query_alma", side_effect=DALServiceError("Query Failed")
        ):
    ...
    def test_fetch_science_types_service_error(self):
        ...
        with patch(
            "app.alma_utils.query_alma", side_effect=DALServiceError("Fetch Failed")
        ):
            with self.assertRaises(DALServiceError) as context:
                fetch_science_types()
    ...
    def test_estimate_alma_beam_size(self):
        ...
        result = estimate_alma_beam_size(
            230 * u.GHz, 1.5 * u.km
        )  # Central frequency in GHz, max baseline in km
    ...
    @patch("builtins.open")
    def test_generate_antenna_config_file_from_antenna_array(
        self, mock_open, mock_read_csv
    ):
        ...
        mock_read_csv.assert_called_once_with(
            os.path.join(
                "test_master_path", "antenna_config", "antenna_coordinates.csv"
            )
        )

    @patch(
        "app.alma_utils.pd.read_csv", side_effect=FileNotFoundError("File not found")
    )
    @patch("builtins.open", new_callable=mock_open)
    def test_generate_antenna_config_file_file_not_found(
        self, mock_open, mock_read_csv
    ):
    ...
    def test_get_max_baseline_from_antenna_config(self, mock_loadtxt):
        ...
        mock_loadtxt.assert_called_once_with(
            "antenna.cfg", usecols=(0, 1, 2), comments="#", dtype=float
        )
    ...
    @patch(
        "app.alma_utils.pd.read_csv",
        return_value=pd.DataFrame(columns=["name", "x", "y", "z"]),
    )
    ...
    def test_query_by_targets_empty_list(self, mock_query):
        ...
        targets = []
        with patch(
            "app.alma_utils.generate_query_for_targets",
            return_value="SELECT * FROM ivoa.obscore WHERE 1=0",
        ):
            result = query_by_targets(targets)
